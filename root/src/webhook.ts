...
import { processSingleCommitComment, checkNewCommitComments, GitHubComment } from './Utils/commitMessage.js';
...
/**
 * Custom Express Request interface to include the raw body buffer for signature verification.
 */
...
/**
 * Interface for the data received from Discord's OAuth2 token exchange.
 */
interface TokenData {
    access_token: string;
...
/**
 * Interface for the basic user data received from Discord's /users/@me endpoint.
 */
 ...
 */
export async function processPushPayload(client: Client, payload: any): Promise<void> {
  try {
 ...
 */
const handleGithubWebhook: RequestHandler = async (req, res, next) => {
    ...
    setImmediate(async () => {
        ...
        const client = req.app.get('client') as Client;
        ...
        try {
            if (event === 'commit_comment' && payload.action === 'created') {
                ...
                await processSingleCommitComment(client, payload.comment as GitHubComment).catch(async err => {
                    ...
                    try { gitHubUpdateGate.enableTemporaryPolling(); } catch (e) { /* best-effort */ }
                });
            } else if (event === 'push') {
                setImmediate(async () => {
                    try {
                        ...
                        try { gitHubUpdateGate.enableTemporaryPolling(); } catch (e) {}
                    }
                });
            } else if (event === 'ping') {
 ...
 */
const handleDiscordStatusWebhook: RequestHandler = async (req, res, next) => {
        ...
        setImmediate(async () => {
            try {
                ...
                for (const subscription of subscriptions) {
                    await handleIncidents(client, subscription, activeIncidents).catch(err => 
                        console.error(`Failed to handle incidents for subscription ${subscription._id}:`, err)
                    );
                }
            } catch (err) {
