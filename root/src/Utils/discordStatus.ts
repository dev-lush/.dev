import { Client, TextChannel, NewsChannel, MessageFlags } from "discord.js";
import fetch from "node-fetch";
import { Subscription, SubscriptionType, IncidentData, ISubscription } from "../Models/Subscription.js";
...
/**
 * @interface Incident
 * @description Represents a single incident or scheduled maintenance from the Discord Status API.
 */
 ...
 * @description Represents a single update within a larger incident.
 */
 ...
 * @description Represents a Discord service or component affected by an incident.
 */
...
function formatComponents(comps: AffectedComponent[]): string {
    ...
    const names = comps.map((c: AffectedComponent) => c.name);
...
async function fetchAllPaginatedData(baseUrl: string, dataKey: 'incidents' | 'scheduled_maintenances', pageLimit: number = Infinity): Promise<any[]> {
    const allItems = [];
    let page = 1;
    let hasMore = true;
    const MAX_RETRIES = 3;
 ...
 */
export async function fetchIncidents(pageLimit: number = 25): Promise<Incident[]> {
    try {
        ...
        const allItemsMap = new Map<string, Incident>();
...
export async function fetchActiveIncidents(): Promise<Incident[]> {
    try {
        ...
        if (!unresolvedRes.ok) console.error(`Failed to fetch unresolved incidents: ${unresolvedRes.status}`);
        if (!activeMaintRes.ok) console.error(`Failed to fetch active maintenances: ${activeMaintRes.status}`);

        const unresolvedData = unresolvedRes.ok ? (await unresolvedRes.json() as StatusApiResponse) : { incidents: [] };
        const activeMaintData = activeMaintRes.ok ? (await activeMaintRes.json() as MaintenanceApiResponse) : { scheduled_maintenances: [] };
 ...
 */
export function buildStatusContainer(incident: Incident, forceOriginalColor = false): any {
    const blocks: any[] = [];
    ...
    if (incident.incident_updates && incident.incident_updates.length > 0) {
        ...
        const groupedUpdates: { status: string; updates: IncidentUpdate[] }[] = [];
        ...
        const statusCounters: Record<string, number> = {}; // Persistent counters for each status type
 ...
 */
export async function getStatusMentionPings(incident: Incident, subscription: ISubscription): Promise<string | undefined> {
    ...
    if (roleMap.has(keysToFetch[0])) pings.add(roleMap.get(keysToFetch[0])!);
    if (roleMap.has(keysToFetch[1])) pings.add(roleMap.get(keysToFetch[1])!);
    if (roleMap.has(keysToFetch[2])) pings.add(roleMap.get(keysToFetch[2])!);
 ...
 */
export function buildStatusPayload(incident: Incident, mention?: string): any {
 ...
 */
export async function handleIncidents(client: Client, subscription: ISubscription, incidents?: Incident[]) {
    try {
        const channel = await client.channels.fetch(subscription.channelId).catch(() => null) as TextChannel | NewsChannel;
        if (!channel) {
                ...
                if (tracked.messageId) {
                    try {
                        ...
                        if (finalIncidentRes.ok) {
                            const responseData = await finalIncidentRes.json() as { incident: Incident };
...
export async function fetchIncidentFeed(): Promise<Incident[]> {
    try {
        // Try RSS first
...
function parseRSSFeed(feed: string): Incident[] {
    // Simple RSS parser
...
function parseAtomFeed(feed: string): Incident[] {
    // Simple Atom parser
    const incidents: Incident[] = [];
...
function getImpactFromDescription(desc: string): string {
    if (desc.toLowerCase().includes('critical')) return 'critical';
    if (desc.toLowerCase().includes('major')) return 'major';
    if (desc.toLowerCase().includes('minor')) return 'minor';
    return 'none';
}

export async function fetchIncidentById(incidentId: string): Promise<Incident | null> {
    try {
