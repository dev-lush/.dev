...
const messagesPath = path.join(__dirname, '..', '..', '..', '..', 'messages', 'docs');
...
interface DocFile {
    ...
    page_meta?: PageMeta[];
    pagination?: {
        previous?: { label: string; emoji?: string };
        next?: { label: string; emoji?: string };
        jump?: { placeholder: string };
    };
    interactions?: {
        [customId: string]: {
...
interface DocMeta {
    ...
    available_tags?: APISelectMenuOption[];
    tag_recommendation_limit?: number;
}
...
/**
 * Reads and caches the documentation files and metadata for a given subcommand.
...
async function getSubcommandData(subcommand: string): Promise<{ files: Map<string, DocFile>, meta: DocMeta }> {
    ...
    const dirPath = path.join(messagesPath, 'docs', subcommand);
    ...
    try {
        ...
        console.error(`Error reading doc directory for subcommand "${subcommand}":`, error);
 ...
 */
async function sendPaginatedRecommendations(
    interaction: StringSelectMenuInteraction,
    subcommand: string,
    matchingDocs: [string, DocFile][]
) {
    let page = 0;
    const itemsPerPage = 5;
    const totalPages = Math.ceil(matchingDocs.length / itemsPerPage);

    const generatePayload = (currentPage: number) => {
        ...
        const menu = new StringSelectMenuBuilder()
            .setCustomId(`docs:${subcommand}:select`)
...
/**
 * Sends a potentially multi-page documentation message and handles all its interactions.
 * @param interaction The interaction to reply to.
 * @param doc The documentation file object to send.
 * @param ephemeral Whether the message should be ephemeral.
 * @param mentionsContent An optional string of mentions to include.
 * @param subcommand The subcommand context.
 */
async function sendDoc(interaction: ChatInputCommandInteraction | StringSelectMenuInteraction, doc: DocFile, ephemeral: boolean, mentionsContent?: string, subcommand?: string) {
    let currentPage = 0;
    const getPage = (index: number) => doc.pages[index];

    const generatePayload = (index: number, finished = false) => {
        const pageData = getPage(index);
        const payload: any = { ...pageData };

        if (finished) {
            payload.components = pageData.components?.map((row: any) => {
                const newRow = { ...row, components: row.components.map((c: any) => ({ ...c, disabled: true })) };
                return newRow;
            });
        } else {
            const components: any[] = payload.components ? [...payload.components] : [];
    ...
    collector.on('end', async () => {
        try {
            ...
            if (error.code !== 10008) { // Unknown Message
                console.error('Failed to disable components on docs command:', error);
            }
        }
    });
}
 ...
 */
async function handleTagSelect(interaction: StringSelectMenuInteraction) {
 ...
 * @param interaction The StringSelectMenuInteraction.
 */
async function handleDocSelect(interaction: StringSelectMenuInteraction) {
    const [, subcommand] = interaction.customId.split(':');
...
// --- Command Definition ---
/**
 * Reads the `messages/docs` directory to find all available documentation categories
...
function getSubcommands(): SubcommandInfo[] {
    const docsPath = path.join(messagesPath, 'docs');
    try {
        const subdirectories = fs.readdirSync(docsPath, { withFileTypes: true })
            .filter(dirent => dirent.isDirectory())
            .map(dirent => dirent.name);

        return subdirectories.map(name => {
            let description = `Documentation for ${name}.`;
            const metaPath = path.join(docsPath, name, '_meta.json');
            try {
        ...
        console.error('Error reading subcommands for /docs:', error);
        return [];
    }
}
 ...
 * Supports fuzzy searching, tag-based searching (e.g., "#tag"), and shows recommendations for empty queries.
 * @param subcommand The subcommand context.
...
async function getDocChoices(subcommand: string, query: string): Promise<{ name: string; value: string }[]> {
    ...
    return Array.from(files.entries())
        .filter(([, fileData]) =>
            fileData.name.toLowerCase().includes(lowerCaseQuery) ||
...
const commandBuilder = new SlashCommandBuilder()
    .setName('docs')
    .setDescription('Display documentation messages.')
...
(async () => {
    try {
                ...
                subcommand
                    ...
                    .addStringOption(option =>
                        option.setName('query')
                            .setDescription('The documentation to search for. Use # for tags.')
                            .setRequired(true)
        ...
        console.error('Failed to build /docs subcommands:', error);
    }
})();
...
export default {
    ...
    /**
     * Handles autocomplete requests for the /docs command.
    ...
    async execute(interaction: ChatInputCommandInteraction<CacheType>) {
        ...
        if (query === '##SELECT_TAGS##') {
            ...
            const menu = new StringSelectMenuBuilder()
                .setCustomId(`docs:${subcommand}:tags`)
        ...
        if (mentionsInput && !ephemeral) {
            ...
            mentionsContent = mentionsInput.split(',')
                .map(m => m.trim())
                .map(mention => {
                    if (mention.toLowerCase() === '@everyone') return '@everyone';
                    if (mention.toLowerCase() === '@here') return '@here';
        ...
        if (!doc) {
            let notFoundMessage: any = { content: '<:Cross:1425291759952593066> The requested document could not be found.', flags: MessageFlags.Ephemeral };
