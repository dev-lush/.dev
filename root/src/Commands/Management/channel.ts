...
const RAW_DESCRIPTIONS: Record<string, string | { default: string; [key: string]: string }> = {
    // General
 ...
 */
function getRelevantPermissions(type: ChannelType) {
    let permNames: Set<string>;
 ...
 */
function buildPermissionOptionsForChannel(targetChannel: GuildChannel, relevantPermissions: { name: string; value: string }[]): APISelectMenuOption[] {
...
async function handleEdit(interaction: ChatInputCommandInteraction) {
    ...
    if (newParent) editOptions.parent = newParent.id;
...
async function showPermsModal(
    ...
    permissionOptions: APISelectMenuOption[],
    uuid?: string
) {
 ...
 * @param interaction The interaction.
 */
async function handlePermissionModal(interaction: ChatInputCommandInteraction) {
    const targetChannel = (interaction.options.getChannel('channel') ?? interaction.channel) as GuildChannel;
    ...
    if (targetChannel.type === ChannelType.GuildVoice || targetChannel.type === ChannelType.GuildStageVoice) {
        ...
        collector.on('collect', async i => {
            ...
            let relevantPerms: { name: string; value: string }[] = [];
            let permNames: Set<string>;
 ...
 */
function getModalSelectValues(interaction: ModalSubmitInteraction, customId: string): string[] {
    try {
        const candidates: any[] = [];

        for (const row of (interaction.components as any[])) {
            if (Array.isArray((row as any).components)) {
                candidates.push(...(row as any).components);
                continue;
            }
            if ((row as any).component) {
                candidates.push((row as any).component);
                continue;
            }
            if ((row as any).customId || (row as any).values) {
                candidates.push(row);
            }
        }

        const extract = (comp: any): string[] | null => {
            if (!comp || typeof comp !== 'object') return null;
            if (Array.isArray(comp.values) && comp.values.length > 0) return comp.values;
            if (typeof comp.value === 'string' && comp.value.length > 0) return [comp.value];
            return null;
        };
 ...
 */
export async function handleChannelPermsModal(interaction: ModalSubmitInteraction) {
