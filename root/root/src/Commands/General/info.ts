...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (subcommand === 'server') {
            ...
            let emoji = '';
            if (features.includes('INTERNAL_EMPLOYEE_ONLY' as any)) emoji = '<:Employee:1413178265278877766>';
            else if (features.includes(GuildFeature.Verified)) emoji = '<:Verified:1411264425431666708>';
            else if (features.includes(GuildFeature.Partnered)) emoji = '<:Partnered:1411264012473339924>';
            else if (features.includes(GuildFeature.Discoverable) && isBoosted) emoji = '<:Discoverable_Boosted:1411264124129906737>';
            else if (features.includes(GuildFeature.Discoverable)) emoji = '<:Discoverable:1411264165149937796>';
            else if (features.includes(GuildFeature.Community) && isBoosted) emoji = '<:Community_Boosted:1411263941010657340>';
            else if (features.includes(GuildFeature.Community)) emoji = '<:Community:1411263844332081262>';
            ...
            // --- Server Assets ---
            const getAssetInfo = (url: string | null, baseName: string) => {
                if (!url) return null;
                return {
            ...
            ].filter((asset): asset is { label: string; type: string } => asset !== null);
            ...
            const components: any[] = [{ type: 17, components: container }];
