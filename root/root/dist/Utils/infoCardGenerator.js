...
import { fontManager } from '../Assets/fonts.js';
import { GUILD_BADGES, BOOST_ICONS, GUILD_STATUS } from '../Assets/guildBadges.js';
/**
...
export class InfoCardGenerator {
     ...
     */
    async generateCard(serverData, options = {}) {
        if (!this.initialized) {
     ...
     */
    async createBannerLayer(serverData) {
     ...
     */
    async createProfileLayer(serverData) {
     ...
     */
    async createServerIcon(serverData) {
     ...
     */
    async createFallbackIcon(serverData) {
     ...
     */
    async createMemberStats(serverData) {
     ...
     */
    async createBoostIndicator(level) {
        const { size } = this.LAYOUT.boost;
        const boostSvgData = BOOST_ICONS[level];
        if (!boostSvgData)
     ...
     */
    async createBadgeImage(badgeType, size) {
        const svgData = GUILD_BADGES[badgeType];
        if (!svgData)
            return Buffer.alloc(0);
     ...
     */
    async createStatusIndicator(isOnline, size) {
     ...
     * @returns An SVG string.
     */
    renderBoostSvg(boostData, size) {
        const paths = boostData.paths.map((p) => `<path d="${p.path}" fill="${p.fill}"></path>`).join('');
     ...
     */
    async createTextSvg(text, options) {
        ...
        const tspanLines = lines.map((line, i) => `<tspan x="0" dy="${i === 0 ? 0 : lineHeight}px">${this.escapeXml(line)}</tspan>`).join('');
        const svg = `
     ...
     */
    async applyFormat(canvas, options) {
