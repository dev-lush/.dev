import { NewsChannel, DiscordAPIError } from 'discord.js';
 ...
 */
const rateLimitedChannels = new Map();
/**
 ...
 */
export async function crosspostMessage(message) {
    try {
        if (!(message.channel instanceof NewsChannel) || !message.crosspostable)
            return;
 ...
 */
export async function republishMessages(client, specificChannelId) {
    const now = Date.now();
    ...
    for (const channelId of channelIds) {
        if (rateLimitedChannels.has(channelId) && !specificChannelId)
            continue;
        let channel;
        try {
        ...
        try {
            ...
            const candidates = Array.from(fetched.values()).filter((msg) => msg.author?.id === client.user?.id &&
                msg.createdTimestamp >= cutoff &&
                msg.interactionMetadata == null && // Skip interaction responses
                msg.crosspostable &&
