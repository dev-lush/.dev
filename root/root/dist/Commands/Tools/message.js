import { SlashCommandBuilder, InteractionCollector, ApplicationIntegrationType, InteractionContextType, MessageFlags } from 'discord.js';
 ...
 */
async function sendDoc(interaction, doc, ephemeral) {
    let currentPage = 0;
    ...
    collector.on('end', async () => {
        try {
            ...
            const disabledPayload = {
                ...latestState,
                components: latestState.components?.map((row) => ({
                    ...row,
                    components: row.components.map((comp) => ({ ...comp, disabled: true })),
                })),
            };
...
export default {
    ...
    async execute(interaction) {
        ...
        if (!doc) {
            let notFoundMessage = { content: '<:Cross:1425291759952593066> The requested message could not be found.', flags: MessageFlags.Ephemeral };
