import { TextChannel, NewsChannel, MessageFlags } from 'discord.js';
 ...
 */
export async function isTokenLoaded() {
...
export async function buildCommitCommentPayload(options, sections) {
    ...
    if (allRolesForSub.length > 0) {
        ...
        const categoryRoles = Object.fromEntries(allRolesForSub
            .filter(r => !r.value.endsWith(':universal'))
            .map(r => [r.value.split(':')[1], `<@&${r.roleId}>`]));
        const pingsSet = new Set();
        if (universalRole) {
    ...
    comps.push({ type: 10, content: `**[@${author}](${authorUrl})** in commit ${shortSha} posted ${verb} comment:` }, { type: 14, spacing: 2 });
    // Prepare attachments
    const files = [];
    ...
    const remainingAttachments = combinedAttachments.filter(att => !processedAttachmentUrls.has(att.url) && !processedAttachmentUrls.has(att.download_url));
 ...
 */
export async function checkNewCommitComments(client) {
    ...
    try {
        ...
        if (!cp) {
            ...
            try {
                ...
                const pickMaxId = (ids) => ids.reduce((m, v) => (v > m ? v : m), 0);
                // Scan recent events
                try {
                    const EVENT_PAGES = 3;
                    const eventCommentIds = [];
                    ...
                    if (maxFromEvents > startCommentId)
                        startCommentId = maxFromEvents;
                }
                catch (e) {
                    console.warn('[init-checkpoint] Events scan failed:', e);
                }
                ...
                try {
                    ...
                    if (commitsRes.ok) {
                        const recentCommits = await commitsRes.json();
                        const commitCommentIds = [];
                        for (const commit of recentCommits) {
                            ...
                            if (commentsOnCommitRes.ok) {
                                const comments = await commentsOnCommitRes.json();
                                comments.forEach(c => commitCommentIds.push(c.id));
                            }
                        }
                        const maxFromCommits = pickMaxId(commitCommentIds);
                        if (maxFromCommits > startCommentId)
                            startCommentId = maxFromCommits;
                    }
                }
                catch (e) {
                    console.warn('[init-checkpoint] Commits scan failed:', e);
                }
        ...
        try {
            const commitsRes = await fetchWithToken(`https://api.github.com/repos/Discord-Datamining/Discord-Datamining/commits?per_page=75`, undefined, false);
            if (commitsRes.ok) {
                const recentCommits = await commitsRes.json();
                for (const commit of recentCommits) {
                    try {
                        const commentsOnCommitRes = await fetchWithToken(`https://api.github.com/repos/Discord-Datamining/Discord-Datamining/commits/${commit.sha}/comments`, undefined, true);
                        if (commentsOnCommitRes.ok) {
                            const commentsOnCommit = await commentsOnCommitRes.json();
        ...
        try {
            ...
            for (let page = 1; page <= MAX_EVENT_PAGES && shouldContinueFetching; page++) {
                ...
                if (events.length < 100)
                    break; // Stop if we process a partial page
            }
        }
        catch (error) {
        ...
        for (const comment of newComments) {
            ...
            try {
                ...
                for (const sub of subs) {
                    if (sub.lastCommentId && comment.id <= sub.lastCommentId)
                        continue;
                    try {
                ...
                try {
                    const res = await CommitCommentCheckpoint.updateOne({ _id: 'global', lastProcessedCommentId: { $lt: comment.id } }, { $set: { lastProcessedCommentId: comment.id } }, { upsert: true });
 ...
 */
export async function processSingleCommitComment(client, comment) {
    ...
    try {
        ...
        if (!commentResponse.ok) {
            ...
            fullComment = await commentResponse.json();
        }
    }
    catch (error) {
    ...
    for (const [chanId, arr] of byChan) {
        ...
        for (const sub of arr) {
            if (sub.lastCommentId && fullComment.id <= sub.lastCommentId)
                continue;
            const allRolesForSub = await RoleMentionsHandler.find({
                guildId: ch.guild.id,
                type: SubscriptionType.PREVIEWS,
    ...
    try {
         ...
         */
        const result = await CommitCommentCheckpoint.updateOne({ _id: 'global', lastProcessedCommentId: { $lt: fullComment.id } }, { $set: { lastProcessedCommentId: fullComment.id } }, { upsert: true } // Creates the document if it doesn't exist
        );
