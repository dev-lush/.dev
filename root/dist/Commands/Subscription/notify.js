import { SlashCommandBuilder, PermissionFlagsBits, PermissionsBitField, RoleSelectMenuBuilder, ComponentType, MessageFlags, InteractionContextType, ApplicationIntegrationType, ButtonBuilder, ButtonStyle } from 'discord.js';
 ...
 */
async function handleRoleSelection(interaction, subscriptionType, customIdParts) {
    ...
    if (!role)
        return interaction.reply({ content: 'No role selected.', flags: MessageFlags.Ephemeral });
    const query = {
 ...
 */
async function showPreviewsCategorySelector(interaction, subId) {
    const container = [
    ...
    async execute(interaction) {
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.isButton()) {
                if (action === 'expand') {
                    if (type === 'previews')
                        await showPreviewsCategorySelector(i, subId);
                    else if (type === 'status')
                        await showStatusImpactSelector(i, subId);
                }
            }
