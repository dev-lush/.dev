...
import { CommitCommentCheckpoint, setCheckpoint, Subscription } from '../../Models/Subscription.js';
...
export default {
    data: new SlashCommandBuilder()
        ...
        .setRequired(true)
        .addChoices({ name: 'View', value: 'view' }, { name: 'Set', value: 'set' }))
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'checkpoint') {
            ...
            if (action === 'view') {
                const checkpoint = await CommitCommentCheckpoint.findById('global').lean();
                if (!checkpoint) {
            ...
            try {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setColor(0x5865F2)
                    .addFields({ name: 'Type', value: sub.type, inline: true }, { name: 'Guild ID', value: `\`${sub.guildId}\``, inline: true }, { name: 'Channel ID', value: `<#${sub.channelId}> (\`${sub.channelId}\`)`, inline: false }, { name: 'User ID', value: `<@${sub.userId}> (\`${sub.userId}\`)`, inline: false }, { name: 'Auto-Publish', value: sub.autoPublish ? '✅ Yes' : '❌ No', inline: true }, { name: 'Created At', value: `<t:${Math.floor(new Date(sub.createdAt).getTime() / 1000)}:F>`, inline: false }, { name: 'Last Permission Warning', value: sub.lastPermissionWarningAt ? `<t:${Math.floor(new Date(sub.lastPermissionWarningAt).getTime() / 1000)}:R>` : 'Never', inline: false });
