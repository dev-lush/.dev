...
const installationIdCache = new Map(); // "owner/repo" -> installationId
...
export async function getInstallationIdForRepo(owner, repo) {
    ...
    const data = await safeParseJSON(txt);
 ...
 */
export async function fetchAsApp(url, init = {}, opts) {
    ...
    if (!installationId) {
        ...
        if (!id)
            throw new Error(`App is not installed on ${opts.owner}/${opts.repo}`);
        installationId = id;
    }
